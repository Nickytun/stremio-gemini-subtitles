...
// gemini-2.5-flash-lite đã bị Google khai tử cho tài khoản mới (HTTP 404) -> thay bằng
// gemini-3.5-flash-lite (chính Google khuyến nghị trong thông báo lỗi).
const DEFAULT_GEMINI_MODELS = ["gemini-flash-latest", "gemini-2.5-flash", "gemini-3.5-flash-lite"];
...
async function translateGeminiBatch(texts, config, keys, depth = 0) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        ...
        const keyLabel = currentKeyIndex % keys.length;
        currentKeyIndex = (currentKeyIndex + 1) % keys.length;
        const model = models[Math.min(modelIndex, models.length - 1)];

        try {
            return await callGemini({ apiKey, model, texts, config });
        } catch (error) {
            lastError = error;
            ...
            if (error.status === 401 || error.status === 403) {
                // Key này hỏng/không hợp lệ cho model này — bỏ qua và thử key/model khác ngay,
                // không cần delay vì đây không phải do quá tải.
                continue;
            }
