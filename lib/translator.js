...
// GIẢM MẠNH BATCH LIMIT ĐỂ GOOGLE KHÔNG BỊ NGHẸN 503
const BATCH_LIMITS = {
...
async function translateGeminiBatchRoundRobin(texts, config, keys, retries = 3) {
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
        ...
        try {
            // Sử dụng bản Lite chuyên xử lý tốc độ cao, chống 503
            ...
            if (response.status === 429) {
                console.log(`[Gemini] Key quá tải (Rate limit), chuyển key tiếp theo...`);
                continue; 
            }
    ...
    // Đã chuyển phần lùi (fallback) ra ngoài vòng lặp, đảm bảo không bao giờ bị lỗi undefined làm sập server
