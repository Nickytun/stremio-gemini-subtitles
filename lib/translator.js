...
function geminiConcurrency(useGemini) {
    // Groq/Mistral free tier có giới hạn request/giây khá chặt (nhất là Mistral) — nếu không còn
    // Gemini (24 key, chịu tải song song tốt) làm tầng chính thì mặc định hạ xuống 1 để tránh dí
    // nhiều batch cùng lúc gây 429 hàng loạt. Vẫn chỉnh được tay qua GEMINI_CONCURRENCY.
    const fallbackDefault = useGemini ? 3 : 1;
    const value = Number(process.env.GEMINI_CONCURRENCY || fallbackDefault);
    return Number.isInteger(value) && value > 0 ? Math.min(value, 8) : fallbackDefault;
}
...
async function translateCues(cues, config, apiKeys = [], onProgress) {
    ...
    const useGemini = keys.length > 0 && !geminiDisabled;
    ...
    if (typeof onProgress === "function") {
        try {
            onProgress(0, batches.length, translated);
        } catch {
            // không để lỗi ở callback báo tiến độ làm hỏng cả quá trình dịch
        }
    }
    ...
    async function runBatch(job) {
        ...
        completedBatches += 1;
        // Báo tiến độ THẬT (đã xong bao nhiêu / tổng) ra ngoài, thay vì để phía trên phải đoán
        // mò bằng thời gian đã trôi qua — người xem cần biết đang dịch có tiến triển hay không,
        // không chỉ "đã chờ bao lâu".
        if (typeof onProgress === "function") {
            try {
                onProgress(completedBatches, batches.length, translated);
            } catch {
                // không để lỗi ở callback báo tiến độ làm hỏng cả quá trình dịch
            }
        }
    ...
    const concurrency = useLlm ? geminiConcurrency(useGemini) : 1;
    let cursor = 0;
...
// Thấy rõ qua cách dùng script Python OCR->AI-edit của người dùng: gọi Gemini TUẦN TỰ + nghỉ
// 2s/lần gần như không bao giờ dính "high demand"/hết quota, trong khi addon này bắn nhiều
// request Gemini gần như đồng thời (nhiều batch song song, mỗi batch lại thử liên tiếp nhiều
// key/model chỉ trong vài trăm ms) — dễ tạo cụm burst khiến Google trả 429/503 nhiều hơn cần
// thiết. Áp dụng đúng triết lý "gọi từ tốn" đó: giãn cách TẤT CẢ request Gemini (dùng chung 1
// mốc thời gian, giống reserveMistralSlot) thay vì để mọi batch tự do bắn cùng lúc.
const GEMINI_MIN_INTERVAL_MS = Number(process.env.GEMINI_MIN_INTERVAL_MS || 400);
let geminiNextAllowedAt = 0;

function reserveGeminiSlot() {
    const now = Date.now();
    const start = Math.max(now, geminiNextAllowedAt);
    geminiNextAllowedAt = start + GEMINI_MIN_INTERVAL_MS;
    return start - now;
}

async function throttleGemini() {
    const wait = reserveGeminiSlot();
...
// Model đôi khi không tuân prompt "chỉ trả mảng chuỗi" mà trả mảng OBJECT (vd {text: "..."}).
// String({...}) trong JS luôn ra đúng chữ "[object Object]" — đây chính là nguyên nhân sub hiện
// ra chữ "[object Object]" dù không có lỗi/exception nào (length đúng nên qua được check).
// Cố đoán field chứa chuỗi thật trong object; không đoán được thì NÉM LỖI để cơ chế chia đôi
// batch (lengthMismatch) tự kích hoạt dịch lại, thay vì âm thầm nhét rác vào phụ đề.
function coerceTranslationValue(value) {
    ...
    if (typeof value === "object") {
        const candidate = value.text ?? value.translation ?? value.vi ?? value.value ?? value.content;
        if (typeof candidate === "string") return candidate;

        const error = new Error("Model trả về phần tử object không đúng định dạng chuỗi (không phải lỗi quota)");
...
function parseTranslationArray(responseText, expectedLength) {
    ...
    return parsed.map(coerceTranslationValue);
}
...
// Groq trả lỗi 429 kèm luôn thời gian chờ thật (vd "Please try again in 14m43.872s") khi chạm
// giới hạn TOKENS PER DAY — đây là lỗi cần NGHỈ THẬT SỰ (không phải quá tải tạm thời như 503 của
// Gemini), đổi key trong cùng model không giúp được gì vì TPD tính theo tổ chức (organization),
// không phải theo key. Đọc số phút/giây trong message để biết chính xác khi nào thử lại, tránh
// mỗi batch lại tốn 4 lần gọi vô ích vào 1 model đã chắc chắn hết quota hôm nay.
const groqModelCooldownUntil = new Map();

function parseGroqRetryAfterMs(message) {
    const match = String(message || "").match(/try again in\s+(?:(\d+)m)?(?:(\d+(?:\.\d+)?)s)?/i);
...
function markGroqModelCooldown(model, ms) {
    const wasOnCooldown = groqModelCooldownUntil.get(model) > Date.now();
    groqModelCooldownUntil.set(model, Date.now() + ms);
    if (!wasOnCooldown) {
        logger.warn("groq model tạm nghỉ do chạm giới hạn free (TPM/TPD)", { model, cooldownMs: ms });
    }
}

function isGroqModelOnCooldown(model) {
    return (groqModelCooldownUntil.get(model) || 0) > Date.now();
}
...
function orderedGroqModels() {
    // Model đang cooldown (hết TPD hôm nay) bị đẩy xuống cuối thay vì loại hẳn — vẫn còn cái để
    // thử nếu MỌI model đều đang nghỉ, nhưng ưu tiên model chưa chạm giới hạn trước.
    const models = groqModels().filter((model) => !deadGroqModels.has(model));
    const alive = models.length ? models : groqModels();
    const fresh = alive.filter((model) => !isGroqModelOnCooldown(model));
    const cooling = alive.filter((model) => isGroqModelOnCooldown(model));
    return fresh.length ? fresh : cooling.length ? cooling : alive;
}
...
async function translateGroqBatch(texts, config, depth = 0) {
    ...
    // Nếu MỌI model đều đang cooldown (hết TPD hôm nay) thì bỏ qua hẳn Groq cho batch này,
    // đừng tốn 1 request nào để rồi chắc chắn ăn 429 lần nữa — nhường ngay cho Mistral.
    if (models.every((model) => isGroqModelOnCooldown(model))) {
        logger.warn("groq: toàn bộ model đang cooldown (chạm giới hạn free), bỏ qua batch này", {
            size: texts.length,
        });
        return null;
    }
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        ...
        try {
            ...
            // 429 = chạm giới hạn free CỦA RIÊNG model này. Groq trả kèm thời gian chờ THẬT
            // (vd "try again in 14m43s") khi là giới hạn TOKENS PER DAY -> ghi nhớ để các batch
            // sau bỏ qua model này ngay, không phí request. Không đọc được thời gian (vd giới hạn
            // RPM tạm thời) thì cooldown ngắn 5s là đủ.
            if (error.status === 429 || error.status === 503) {
                const retryMs = parseGroqRetryAfterMs(error.message);
                markGroqModelCooldown(model, Math.min(retryMs || 5000, 30 * 60 * 1000));
                if (modelIndex < models.length - 1) {
                    modelIndex += 1;
                    continue;
                }
...
// Mistral free (Experiment) không cho biết đúng khi nào hết giới hạn như Groq — lỗi 429 chỉ ghi
// "Rate limit exceeded" trống trơn, gần như chắc chắn là giới hạn REQUEST/GIÂY bị vượt do nhiều
// batch gọi cùng lúc (concurrency). Thay vì tốn request thử-rồi-thất-bại, dùng 1 mốc thời gian
// dùng chung cho MỌI batch: request nào cũng phải đợi tới mốc đó mới được bắn đi, và mỗi lần dính
// 429 thì đẩy mốc ra xa thêm — giống hàng đợi giãn cách tốc độ (rate limiter) đơn giản.
// Free tier "Experiment" của Mistral có vẻ chặt hơn 400ms/request nhiều — nâng mặc định lên 1.5s.
const MISTRAL_MIN_INTERVAL_MS = Number(process.env.MISTRAL_MIN_INTERVAL_MS || 1500);
let mistralNextAllowedAt = 0;

// BUG ĐÃ SỬA: bản cũ đọc mistralNextAllowedAt rồi mới await delay() TRƯỚC KHI cập nhật lại biến
// -> nhiều batch chạy song song (concurrency>1) có thể cùng đọc thấy "được phép gọi ngay" trong
// cùng 1 tick, cùng bắn request, vẫn dính 429 dù có throttle. Vì JS single-thread, "đặt chỗ"
// (tính slot kế tiếp rồi ghi lại biến) phải làm ĐỒNG BỘ, không xen await ở giữa, mới chặn được
// race này.
function reserveMistralSlot() {
    const now = Date.now();
    const start = Math.max(now, mistralNextAllowedAt);
    mistralNextAllowedAt = start + MISTRAL_MIN_INTERVAL_MS;
    return start - now;
}

async function throttleMistral() {
    const wait = reserveMistralSlot();
    if (wait > 0) await delay(wait);
}

function backoffMistral(ms = 2500) {
    mistralNextAllowedAt = Math.max(mistralNextAllowedAt, Date.now() + ms);
}
...
async function callMistral({ apiKey, model, texts, config }) {
    ...
    return array.map(coerceTranslationValue);
}
...
async function translateMistralBatch(texts, config, depth = 0) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        ...
        try {
            ...
            if (error.status === 429 || error.status === 503) {
                // Không có model dự phòng thứ 2 để "trốn" như Groq (chỉ 2 model, cả hai cùng
                // chung 1 giới hạn request/giây của tài khoản) -> đẩy mốc throttle ra xa rồi thử
                // lại đúng model đó, thay vì đổi model (không giúp được gì với lỗi RPS).
                backoffMistral();
                continue;
            }
