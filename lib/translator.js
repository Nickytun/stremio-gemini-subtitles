...
function markGroqModelCooldown(model, ms) {
    ...
    if (!wasOnCooldown) {
        logger.warn("groq model tạm nghỉ do hết quota ngày (TPD)", { model, cooldownMs: ms });
    }
}
...
async function translateGroqBatch(texts, config, depth = 0) {
    ...
    if (models.every((model) => isGroqModelOnCooldown(model))) {
        logger.warn("groq: toàn bộ model đang cooldown (hết TPD hôm nay), bỏ qua batch này", {
            size: texts.length,
        });
        return null;
    }
...
const MISTRAL_MIN_INTERVAL_MS = Number(process.env.MISTRAL_MIN_INTERVAL_MS || 400);
let mistralNextAllowedAt = 0;

async function throttleMistral() {
    const wait = mistralNextAllowedAt - Date.now();
    if (wait > 0) await delay(wait);
    mistralNextAllowedAt = Date.now() + MISTRAL_MIN_INTERVAL_MS;
}
