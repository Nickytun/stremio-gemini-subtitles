...
// Slug model free cụ thể (vd DeepSeek/Qwen/Llama) rớt hạng free RẤT nhanh — log thực tế cho thấy
// cả 3 slug từng thêm vào đây đã hết free chỉ sau vài ngày. Vì vậy mặc định chỉ dùng
// "openrouter/free" — auto-router của chính OpenRouter, luôn tự chọn 1 model đang free tại thời
// điểm gọi nên không bao giờ bị lỗi thời. Muốn ưu tiên thử model cụ thể trước (để kiểm soát chất
// lượng) thì đặt biến môi trường OPENROUTER_MODEL, vd: "qwen/qwen3-8b:free,deepseek/deepseek-v3.2:free"
// — cứ đặt tên gì cũng được, model nào hết free sẽ tự bị loại vĩnh viễn sau 1 lần thử (xem
// markOpenrouterModelDead), không cần sửa code mỗi lần OpenRouter đổi danh sách free.
const DEFAULT_OPENROUTER_MODELS = ["openrouter/free"];

let currentOpenrouterKeyIndex = 0;

// Slug free trên OpenRouter rớt hạng rất nhanh (nhà cung cấp đổi model free hàng tuần) — khi 1 model
// trả 404 "unavailable for free", đó là lỗi VĨNH VIỄN (không phải quá tải tạm thời), nên nhớ lại
// (dùng chung cho mọi batch, giống deadKeys của Gemini) để các batch sau bỏ qua ngay, không thử lại.
// "openrouter/free" (auto-router của chính OpenRouter) không bao giờ bị loại vì nó luôn tự chọn
// model đang free tại thời điểm gọi.
const deadOpenrouterModels = new Set();

function markOpenrouterModelDead(model) {
    if (model === "openrouter/free") return;
    if (!deadOpenrouterModels.has(model)) {
        deadOpenrouterModels.add(model);
        logger.warn("openrouter model bị loại vĩnh viễn (hết free/không tồn tại)", { model });
    }
}

function orderedOpenrouterModels() {
    const models = openrouterModels();
    const alive = models.filter((model) => !deadOpenrouterModels.has(model));
...
async function translateOpenRouterBatch(texts, config, depth = 0) {
    ...
    const models = orderedOpenrouterModels();
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        ...
        try {
            ...
            // 404 = model đã hết free/không còn tồn tại -> lỗi VĨNH VIỄN, đổi model khác không cần đợi.
            if (error.status === 404) {
                markOpenrouterModelDead(model);
            ...
            // 429/503 = quá tải TẠM THỜI -> vẫn còn cơ hội, đổi model dự phòng rồi đợi nếu hết model.
