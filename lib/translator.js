...
const BATCH_LIMITS = {
    ...
    gemini: {
        chars: 30000,
        texts: 80, // Gom 80 câu/lần để Gemini xử lý nhanh, mượt và không bị ngắt quãng
    },
};
...
async function translateCues(cues, config, apiKeys = []) {
    ...
    // Ép hệ thống dùng giới hạn batch của Gemini nếu có truyền apiKeys
...
async function translateGeminiBatchRoundRobin(texts, config, keys, retries = 3) {
    for (let attempt = 0; attempt < retries; attempt++) {
        ...
        // Thuật toán Round-robin: Xoay vòng mảng Key liên tục
