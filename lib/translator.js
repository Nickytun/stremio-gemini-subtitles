...
async function translateGeminiBatchRoundRobin(texts, config, keys, retries = 5) {
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
        ...
        try {
            ...
            // Đã nhúng biến config.googleSourceLanguage để AI biết chính xác gốc là tiếng Trung (zh/chi) hay tiếng Anh (en)
            const prompt = `Bạn là biên dịch viên phụ đề chuyên nghiệp chuẩn Netflix/CBS. Nhiệm vụ của bạn là dịch mảng văn bản phụ đề sau từ ngôn ngữ gốc (mã ngôn ngữ: ${config.googleSourceLanguage || 'auto'}) sang Tiếng Việt. 
            LƯU Ý QUAN TRỌNG: Nếu ngôn ngữ gốc là tiếng Trung, hãy dịch thoát ý, ngữ cảnh tự nhiên, tuyệt đối không dịch kiểu Hán-Việt cứng nhắc hay word-by-word.
            
            YÊU CẦU BẮT BUỘC:
            1. Văn Phong & Ngôn Ngữ: Sử dụng từ ngữ, câu văn của miền Nam Việt Nam. TUYỆT ĐỐI dùng từ "vậy" thay cho "thế" (VD: "sao vậy", "như vậy").
            ...
            4. Thẻ âm thanh: Giữ nguyên 100%, dịch sang tiếng Việt và viết thường trong ngoặc vuông (VD: [nhạc vui nhộn], [tiếng súng]).
