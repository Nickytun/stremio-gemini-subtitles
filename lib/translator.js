...
async function translateGeminiBatchRoundRobin(texts, config, keys, retries = 3) {
    // Ép kiểu: Dù nhập chuỗi trần hay mảng thì nó đều gom lại thành mảng chuẩn chỉnh
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
        ...
        try {
            // Chuyển sang dùng bản latest để tránh lỗi 404 Model
            ...
            if (!response.ok) {
                // Ép nhả lỗi gốc của Google ra log để dễ bắt bệnh
